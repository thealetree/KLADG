...
export default function QueueView({ queue, upcomingTracks, onRemove, onClear, artMap }) {
  return (
    <div className="queue-panel">
      ...
      {upcomingTracks && upcomingTracks.length > 0 ? (
        <>
          <div className="queue-header">
            <span className="queue-header-label">Up next</span>
          </div>
          <div className="queue-list-full">
            {upcomingTracks.map(track => (
              <div key={track.id} className="queue-item queue-item-upcoming">
                <img
                  className="queue-item-art"
                  src={`/art/${artMap[track.artId]}`}
                  alt=""
